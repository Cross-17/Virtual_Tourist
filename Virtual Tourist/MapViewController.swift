...
class MapViewController: UIViewController,MKMapViewDelegate {
    ...
    func loadPinsFromDatabase()
    {
        ...
        {
            let annotation = MKPointAnnotation()
            annotation.coordinate = CLLocationCoordinate2D(latitude: CLLocationDegrees(item.lat),longitude: CLLocationDegrees(item.lot))
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        do
        {
            ...
            let predicate = NSPredicate(format: "lat == %@ AND lot == %@", argumentArray: [selected.coordinate.latitude, selected.coordinate.longitude])
